...
export function Hero() {
  return (
    <section id="hero" className="relative flex min-h-svh items-center overflow-hidden">
      ...
      {/* グリッド */}
      <div aria-hidden="true" data-print="hide" className="bg-grid absolute inset-0" />
      ...
      >
        {engineerHero.available && (
          ...
          >
            <span className="relative flex h-2 w-2" data-print="hide">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75" />
              <span className="relative inline-flex h-2 w-2 rounded-full bg-emerald-400" />
            </span>
            {engineerHero.availableText}
          </motion.div>
        )}
        ...
        >
          {engineerHero.role} · {engineerHero.location}
        </motion.p>

        <h1 className="font-display text-[clamp(2.75rem,9vw,6.5rem)] font-bold leading-[0.98] tracking-tight">
          <motion.span variants={item} className="block">
            {engineerHero.tagline[0]}
          </motion.span>
          <motion.span variants={item} className="block">
            <span className="text-gradient animate-shimmer">{engineerHero.tagline[1]}</span>
          </motion.span>
        </h1>
