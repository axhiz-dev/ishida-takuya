...
`/` の左パネルに出る「6 年 / React · TypeScript · JavaScript」と、`/engineer` の
ヘッダに出る「6年 エンジニア経験」は、**どちらも `engineerCareer` の
在籍期間（`kind: "sales"` を除く）から計算しています。** 書き換える場所はありません。
日付が進めば自動で増えます。

「15+ 携わったプロジェクト」などほかの数字は導出できないので
`engineerStats` に手で書いてあります。
...
`/engineer` の右下にある「PDF出力」を押すと、ブラウザの印刷ダイアログが開きます。
